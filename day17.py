...
def draw(cave_width, solidified, inflight: Tuple[int, int, Shape], draw_whole=False):
    ...
    def is_inflight(x: int, y: int) -> bool:
        ...
        return (x, y) in ((px + inflight_x, inflight_y - py) for px, py in shape.points())
...
def can_move_left(shape_x, shape_y, shape, solidified):
    return 0 < shape_x and \
        not collides(shape_x - 1, shape_y, shape, solidified)

def can_move_right(shape_x, shape_y, shape, solidified):
    cave_width = 7
    return shape_x < cave_width - shape.width \
        and not collides(shape_x + 1, shape_y, shape, solidified)

def can_fall_further(shape_x, shape_y, shape, solidified):
    return not on_ground(shape_y, shape) and \
        not collides(shape_x, shape_y - 1, shape, solidified)
    ...
    for i, shape in enumerate(islice(falling_shapes(), 2022)):
        ...
        while True:
            ...
            if direction == '<' and can_move_left(shape_x, shape_y, shape, solidified):
                shape_x -= 1
            elif direction == '>' and can_move_right(shape_x, shape_y, shape, solidified):
                shape_x += 1
